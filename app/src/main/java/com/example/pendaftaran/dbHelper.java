...
public class dbHelper extends SQLiteOpenHelper {
    ...
    public dbHelper(Context context) {
        ...
        super(context, "input.db", null, 3);
    }

    @Override
    public void onCreate(SQLiteDatabase DB) {
        DB.execSQL("Create Table Pendaftaran( Nama TEXT Primary Key , " +
                "Alamat Text, NoHP Number, JenisKelamin String)");
    ...
    public Boolean insertpendaftaran(String key_nama, String key_alamat, String key_nohp) {
        ...
        long result = DB.insert("pendaftaran", null, contentValues);
        if (result == -1) {
            return false;
        } else {
            return true;
